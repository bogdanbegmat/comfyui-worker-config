...
# 1.1 Configure pip globally – no local wheel cache, no version banner.
...
# 3. Install git (needed for repo clones).
...
#    * Removes the pip cache automatically via ENV.
#    * Drops duplicate / huge wheels we don't actually need.
RUN --mount=type=cache,target=/root/.cache/pip \
    # prune lines that would bloat or break the build ----------------------- \
...
# 12. Use the same entrypoint as the base image (inherits CMD/ENTRYPOINT)
